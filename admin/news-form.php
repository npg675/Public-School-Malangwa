...
if ($editing) { try { $stmt = $pdo->prepare('SELECT * FROM news WHERE id = ?'); $stmt->execute([(int)$_GET['id']]); $row = $stmt->fetch(); } catch (Throwable $e) { error_log('News load failed: '.$e->getMessage()); $flash=['err','News could not be loaded. Check the database connection.']; } if (!$row && !$flash) { header('Location: '.base_url('admin/news.php')); exit; } }
if ($_SERVER['REQUEST_METHOD'] === 'POST' && csrf_verify($_POST['_csrf'] ?? '')) {
    $d = ['title_en'=>trim($_POST['title_en']??''),'title_np'=>trim($_POST['title_np']??''),'slug'=>trim($_POST['slug']??''),'category_id'=>$_POST['category_id']?:null,'excerpt_en'=>trim($_POST['excerpt_en']??''),'excerpt_np'=>trim($_POST['excerpt_np']??''),'content_en'=>$_POST['content_en']??'','content_np'=>$_POST['content_np']??'','cover_image'=>trim($_POST['cover_image']??''),'published_at'=>$_POST['published_at']??date('Y-m-d H:i:s'),'status'=>in_array($_POST['status']??'draft',['draft','published'],true) ? $_POST['status'] : 'draft'];
    if (empty($d['title_en'])) { $flash = ['err','Title required.']; }
    else {
        if (empty($d['slug'])) $d['slug'] = strtolower(trim(preg_replace('/[^a-zA-Z0-9]+/','-',$d['title_en']),'-'));
        try { if ($editing) { $s=[]; foreach($d as $k=>$v) $s[]="`$k`=:$k"; $d[':id']=(int)$_GET['id']; $pdo->prepare('UPDATE news SET '.implode(', ',$s).' WHERE id=:id')->execute($d); $flash=['ok','News updated.']; } else { $d['created_by']=$_SESSION['user_id']??null; $c=implode('`, `',array_keys($d)); $v=':'.implode(', :',array_keys($d)); $pdo->prepare("INSERT INTO news (`$c`) VALUES ($v)")->execute($d); $flash=['ok','News created.']; } } catch (Throwable $e) { error_log('News save failed: '.$e->getMessage()); $flash=['err','News could not be saved. Check the slug and database connection.']; }
    }
}
?>
