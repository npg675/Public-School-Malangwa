...
$items = [];
if ($pdo && db_has_table('staff')) { try { $items = $pdo->query("SELECT s.*, c.name_en as cat_en FROM staff s LEFT JOIN staff_categories c ON c.id=s.category_id ORDER BY c.sort_order, s.display_order, s.name_en")->fetchAll(); } catch (Throwable $e) {} }
?>
...
<?php else: foreach ($items as $s): ?><tr>
    <td><?php if(!empty($s['photo'])): ?><img src="<?= e_attr(base_url($s['photo'])) ?>" style="width:40px;height:40px;border-radius:50%;object-fit:cover" onerror="this.style.display='none'"><?php else: ?><div style="width:40px;height:40px;border-radius:50%;background:#E2E8F0;display:grid;place-items:center;font-weight:700;color:#667085;font-size:.8rem"><?= strtoupper(substr($s['name_en'],0,1)) ?></div><?php endif; ?></td>
